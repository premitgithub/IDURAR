...
export const crud = {
  ...
  delete:
    ...
    //   const keyState = KEY_STATE.delete;
    // dispatch({ type: actionTypes.REQUEST_LOADING, keyState });

    // try {
    //   const response = await request.delete(`/${entity}/${id}`);
    //   dispatch({ type: actionTypes.REQUEST_SUCCESS, keyState, payload: response.data });
    // } catch (error) {
    //   dispatch({ type: actionTypes.REQUEST_FAILED, keyState });
      }
    },
